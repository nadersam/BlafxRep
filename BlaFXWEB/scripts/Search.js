...
$(document).ready(function () {
    ...
    vm.Execute4("https://crm.blafx.com/crm/followConsumer/getOrder?mt5Login=5506597&orderType=1");
    vm.Execute2("https://crm.blafx.com/crm/followConsumer/getOrder?mt5Login=1001015&orderType=1");
    vm.Execute3("https://crm.blafx.com/crm/followConsumer/getOrder?mt5Login=5501835&orderType=1");
    ko.applyBindings(vm);
});
...
var MainViewModel = function () {
    ...
    self.Stats4 = ko.observableArray([]);
    self.Results4 = ko.observableArray([]); 
    self.Stats2 = ko.observableArray([]);
    self.Results2 = ko.observableArray([]);   
    self.Stats3 = ko.observableArray([]);
    self.Results3 = ko.observableArray([]);
    ...
    self.Execute4 = function (URL) {              
        $.ajax({
            ...
            success: function (data) {
                ...
                self.Results4(dataMapped);
                if (isToday(self.Results4()[0].open_time()) == true) {
                    document.getElementById('ark4label').style.backgroundColor = 'green';
                }
                ko.utils.arrayForEach(vm.Results4(), function (Res) {
                    if (vm.Stats4().length == 0) {
                        vm.Stats4.push(new StatsEntity({ Hour: Res.OpenHour(), Count: 1 }));
                    }
                    var found = false;
                    for (var xx = 0; xx <= vm.Stats4().length - 1; xx++) {                      
                        if (vm.Stats4()[xx].Hour() == Res.OpenHour()) {
                            found = true;
                            break;
                        }                       
                    }
                    if (found == false) {
                        ...
                        vm.Stats4()[xx].Count(parseInt(vm.Stats4()[xx].Count()) + 1);
                    }
                });
                vm.Stats4(vm.Stats4().sort((a, b) => (a.Count() > b.Count()) ? -1 : ((b.Count() > a.Count()) ? 1 : 0)));
            },
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (isToday(self.Results2()[0].open_time()) == true) {
                    document.getElementById('ark2label').style.backgroundColor = 'green';
                }
    ...
    self.Execute3 = function (URL) {
        $.ajax({
            ...
            success: function (data) {
                var dataMapped = $.map(data.rows, function (item) { return new DrugsSearchListEntity(item); });
                self.Results3(dataMapped);
                if (isToday(self.Results3()[0].open_time()) == true) {
                    document.getElementById('ark3label').style.backgroundColor = 'green';
                }
                ko.utils.arrayForEach(vm.Results3(), function (Res) {
                    if (vm.Stats3().length == 0) {
                        vm.Stats3.push(new StatsEntity({ Hour: Res.OpenHour(), Count: 1 }));
                    }
                    var found = false;
                    for (var xx = 0; xx <= vm.Stats3().length - 1; xx++) {
                        if (vm.Stats3()[xx].Hour() == Res.OpenHour()) {
                            found = true;
                            break;
                        }
                    }
                    if (found == false) {
                        ...
                        vm.Stats3()[xx].Count(parseInt(vm.Stats3()[xx].Count()) + 1);
                    }
                });
                vm.Stats3(vm.Stats3().sort((a, b) => (a.Count() > b.Count()) ? -1 : ((b.Count() > a.Count()) ? 1 : 0)));
                setTimeout(function () {
                    self.GenerateCompareData();
                }, 2000);
